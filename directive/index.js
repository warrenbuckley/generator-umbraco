...
var UmbracoGenerator = yeoman.generators.NamedBase.extend({
  initializing: function () {
    ...
    if(!configData){
    	this.log(chalk.red('No config data found. Please ensure you run yo umbraco first before calling yo umbraco:directive'));
      ...
      this.directiveName     = configData.names.subgenerators.directive + this.name;
      this.directiveFileName = configData.names.subgenerators.directive + this.name + '.js';
    }
  ...
  writing: function () {
    ...
    if(!this.hasErrors){
      ...
      this.log(chalk.green('Create a new directive: ' + this.directiveName));

      //Copy & template directive file
      this.template('name.directive.js', 'app/scripts/directives/' + this.directiveFileName);
    }
  }
});
